...
앱은 과거 시계열을 저장하거나 공급자 사용량을 무조건 폴링하지 않습니다. 시작·수동 새로고침 때 현재 스냅샷만 읽고 Codex App Server는 즉시 종료하며, 작업표시줄 표시가 켜진 경우에만 Claude 초기화 시각 경과를 감시해 필요한 순간에 재조회합니다. 자세한 기준은 [`docs/performance/memory-budget.md`](./docs/performance/memory-budget.md)에 있습니다.
