...
이 폴더는 iOS와 macOS 앱에서 공유할 수 있는 Swift 네이티브 인증 경계입니다.
...
3. 공급자별 authorize URL과 client ID는 앱 환경별 설정으로 주입하고, client secret은 앱 번들에 넣지 않습니다.
4. callback 성공 뒤 authorization code 교환과 `SpectraKeychainCredentialVault.write` 호출은 공급자 adapter가 네이티브 코드에서 수행합니다.
