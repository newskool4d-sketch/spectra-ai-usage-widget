...
| 축 | 기준 | 측정 방법 |
|---|---|---|
| 표시 | 스트립 켬 상태에서 작업표시줄 알림 영역 왼쪽에 `[Codex CI] NN% · [Claude CI] 5h NN% · 7d NN%`가 창 없이 보인다 | 스크린샷 또는 사용자 육안 확인 |
...
표시·위치·테마·기본값·Explorer 재시작·전체화면/자동숨김 전환은 Task 2~5 자동 검사와 사용자 육안 확인으로 PASS했다. 대기 상태 메모리는 Task 6에서 61개 표본 평균 33.6 MB, 최대 34.1 MB, 프로세스 수 1개로 PASS했다. 갱신 정책은 2026-09-18 스펙(2분 주기 갱신)으로 대체됐고 `Cargo.lock`의 새 package는 없다. 별도 D3D 전체화면, 물리 다중 모니터, 30분 가려짐 관찰은 이 판정에 포함하지 않았다.
...
### 3-3. 공급자별 표시 값 (2026-09-23 개정: Claude 5시간·주간 동시 표시)

Claude는 최소 잔여율 하나를 표시하면 5시간·주간 중 표시 기준이 바뀌어 혼동을 준다. 2026-09-23 사용자 선택에 따라 **`5h NN% · 7d NN%`를 고정 순서로 함께 표시**한다. 창 ID `rolling`·`weekly`로 각각 선택하며 데이터 순서나 잔여율 크기에 따라 자리를 바꾸지 않는다. Codex는 기존대로 자기 한도 창 중 최소 잔여율 하나를 표시한다.

| 상태 | 표시 |
|---|---|
| Codex `connected`·`stale` + 창 1개 이상 | 최소 잔여율 `NN%` |
| Claude `connected`·`stale` | `5h NN% · 7d NN%`, 각 창의 잔여율·색상을 독립 적용 |
| Claude 한쪽 창 누락 | 해당 자리만 `—`, 다른 기간 값으로 대체하지 않음 |
| 그 외(로그아웃·미설치·창 없음) | Codex `—`, Claude `5h — · 7d —` |
| 두 공급자 모두 표시할 값 없음 | 스트립 창을 숨김(빈 막대를 띄우지 않음) |
...
- Claude CI는 한 번만 표시하고 뒤에 `5h`·`7d` 라벨과 잔여율을 붙인다. 기간 라벨은 중립색, 각 잔여율은 해당 값의 경고 색을 사용한다.
...
### 3-6. 갱신 시점 (2026-09-18 개정: 표시 중 2분 주기 갱신)
...
2026-09-12 판정의 "무조건 폴링 금지"는 2026-09-18 진단(스트립이 사용자 조작 없이는 갱신되지 않음)으로 폐기했다. 표시 중에만 주기 갱신하며, 표시가 꺼지면 현행대로 조회하지 않는다.
