...
Windows 테스트는 툴팁 컨트롤 문자열 소비·갱신·hover 메시지 연결·숨김·UTF-16 버퍼 수명·창 10회 생성/파기를 검증합니다. 최초 검증에서는 잠금 화면으로 실제 UI 확인을 보류했지만, 이후 잠금 해제 후 앱 내 업그레이드·새 버전 화면·작업 표시줄의 실제 마우스 hover 표시를 모두 확인했습니다.
...
## 후속 릴리즈 전체 자동 배포 (v0.2.3)

- 태그 `v0.2.3`을 푸시해 [GitHub Actions 실행 34816447140](https://github.com/newskool4d-sketch/spectra-ai-usage-widget/actions/runs/34816447140)을 시작했습니다.
- `Verify release version`, 서명 키 설정 확인, 프런트 테스트, Rust 기본 테스트, NSIS 빌드, 초안 자산 검증, 체크섬 업로드, 릴리즈 공개 전환이 모두 통과했습니다. 전체 실행 시간은 10분 8초입니다.
- 릴리즈는 `draft=false`, `prerelease=false`이며 최신 태그가 `v0.2.3`입니다. 공개 시각은 `2026-09-14 16:18:26 KST`입니다.
- 공개 자산 4개(`SPECTRA_0.2.3_x64-setup.exe`, `.sig`, `latest.json`, `SHA256SUMS.txt`)가 모두 업로드됐습니다.
- 공개 설치 파일은 2,834,074 bytes, SHA-256 `473441403DF62EEB7794E5DDBE7511BA51C4A75E86F0F0BE910F3FFD2BC93CF6`이며 GitHub asset digest와 일치합니다.
- 공개 `latest.json`의 버전은 `0.2.3`이고, 서명·manifest·업로드 digest 검증을 `node scripts/verify-release.mjs <download-dir>`로 통과했습니다.
...
- 사용자가 작업 표시줄에서 실제 마우스 hover 표시를 확인했습니다. 네이티브 컨트롤 테스트와 실제 육안 확인을 모두 완료한 것으로 구분해 기록합니다.
...
| 3. 실제 설치 화면의 마우스 hover 육안 확인 | PASS | 사용자가 작업 표시줄에서 실제 포인터 hover 표시를 확인 |
| 후속 릴리즈의 전체 CI 자동 공개 | PASS | v0.2.3 태그 실행에서 테스트·빌드·서명 자산 검증·체크섬 업로드·공개 전환 모두 통과 |
...
코드 테스트·서명 검사·공개 다운로드 성공은 실제 앱 내 업그레이드 시험을 대신하지 않습니다. 이번 범위에서는 실제 업그레이드·재시작·설정 보존·작업 표시줄 hover 육안 확인과 후속 v0.2.3 전체 자동 공개까지 완료했습니다. v0.2.3 공개 설치 파일의 별도 신규 설치 화면 확인은 이번 자동 배포 검증 범위에 포함하지 않았습니다.
