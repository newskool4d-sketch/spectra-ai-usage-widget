...
| 후속 릴리즈의 전체 CI 자동 공개 | 진행 예정 | v0.2.3 태그 푸시 후 Actions 전체 실행으로 확인 |
...
코드 테스트·서명 검사·공개 다운로드 성공은 실제 앱 내 업그레이드 시험을 대신하지 않습니다. 이번에는 실제 업그레이드·재시작·설정 보존·작업 표시줄 hover 육안 확인까지 완료했으며, 후속 릴리즈 전체 CI 자동 공개는 v0.2.3 실행 결과로 확정합니다.
