...
- 작업표시줄 스트립은 Computer Use의 선택 가능한 창 목록에 노출되지 않아 사용자 육안 확인으로 보완했다. 2026-09-23 사용자가 `5h`·`7d` 표시와 주변 아이콘 겹침 여부에 “문제 없음”을 확인했다.
...
| 작업표시줄 실물 표시·주변 아이콘 간격 | PASS | 2026-09-23 사용자 육안 확인: 두 값 표시·주변 아이콘 겹침 문제 없음 |

전체 판정: **PASS** — 빌드·공개 배포·실제 앱 업데이트·설정 보존과 작업표시줄 실물 표시·간격 확인을 완료했다.
